...
final class ScreenLockController: NSObject, ObservableObject {
    ...
    /// Increments even when the semantic presentation is unchanged. WindowManager
    /// uses it to reattach the panel after sleep, wake and secure-space swaps.
    ...
    func stop() {
        ...
        workspaceCenter.removeObserver(self, name: NSWorkspace.screensDidSleepNotification, object: nil)
        workspaceCenter.removeObserver(self, name: NSWorkspace.screensDidWakeNotification, object: nil)
        dismissTask?.cancel()
        dismissTask = nil
    ...
    @objc private func handleScreensDidWake(_ notification: Notification) {
        guard settings.lockUnlockAnimationEnabled,
              currentPresentation != nil else { return }
        lifecycleRevision &+= 1
    }
