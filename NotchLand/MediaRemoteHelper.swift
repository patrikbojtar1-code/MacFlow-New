...
final class MediaRemoteHelper: ObservableObject {
    ...
    typealias GetApplicationPIDFn = @convention(c) (DispatchQueue, @escaping (Int32) -> Void) -> Void
    ...
    let getApplicationPID: GetApplicationPIDFn? = CFBundleGetFunctionPointerForName(
    ...
    @Sendable
    func printPayload(_ payload: [String: Any]) {
    ...
    @Sendable
    func emitWithSource(_ payload: [String: Any]) {
        guard let getApplicationPID else {
            printPayload(payload)
            return
        }
        getApplicationPID(.main) { pid in
            var enriched = payload
            if let application = NSRunningApplication(processIdentifier: pid_t(pid)) {
                if let bundleIdentifier = application.bundleIdentifier {
                    enriched["sourceBundleIdentifier"] = bundleIdentifier
                }
                if let localizedName = application.localizedName {
                    enriched["sourceApplication"] = localizedName
                }
            }
            printPayload(enriched)
        }
    }
    ...
    func emit() {
        getInfo(.main) { info in
            ...
            if let v = info["kMRMediaRemoteNowPlayingInfoGenre"] as? String { out["genre"] = v }
            if let v = info["kMRMediaRemoteNowPlayingInfoServiceIdentifier"] as? String { out["serviceIdentifier"] = v }
            if let v = info["kMRMediaRemoteNowPlayingInfoExternalContentIdentifier"] as? String { out["contentIdentifier"] = v }
            if let v = info["kMRMediaRemoteNowPlayingInfoContentItemIdentifier"] as? String { out["contentIdentifier"] = v }
            if let v = number(info["kMRMediaRemoteNowPlayingInfoContentItemIdentifier"]) { out["contentIdentifier"] = String(format: "%.0f", v) }
            if let v = info["kMRMediaRemoteNowPlayingInfoiTunesStoreIdentifier"] as? String { out["storeIdentifier"] = v }
            if let v = number(info["kMRMediaRemoteNowPlayingInfoiTunesStoreIdentifier"]) { out["storeIdentifier"] = String(format: "%.0f", v) }
            if let v = info["kMRMediaRemoteNowPlayingInfoAssetURL"] as? String { out["contentURL"] = v }
            if let v = info["kMRMediaRemoteNowPlayingInfoUniqueIdentifier"] as? String { out["uniqueIdentifier"] = v }
            if let v = info["kMRMediaRemoteNowPlayingInfoMediaType"] as? String { out["mediaType"] = v }
            if let v = number(info["kMRMediaRemoteNowPlayingInfoMediaType"]) { out["mediaTypeCode"] = v }
            if let v = number(info["kMRMediaRemoteNowPlayingInfoTrackNumber"]) { out["trackNumber"] = v }
            if let v = number(info["kMRMediaRemoteNowPlayingInfoTotalTrackCount"]) { out["totalTrackCount"] = v }
            ...
            // Some Apple video sources publish richer, private semantic keys.
            // Normalize only the TV fields we understand instead of forwarding
            // the entire MediaRemote dictionary to the host process.
            for (rawKey, rawValue) in info {
                let key = rawKey.lowercased()
                if key.contains("seriesname") || key.contains("showname") {
                    if let value = rawValue as? String { out["seriesTitle"] = value }
                } else if key.contains("episodetitle") {
                    if let value = rawValue as? String { out["episodeTitle"] = value }
                } else if key.contains("seasonnumber") {
                    if let value = number(rawValue) { out["seasonNumber"] = value }
                } else if key.contains("episodenumber") {
                    if let value = number(rawValue) { out["episodeNumber"] = value }
                } else if key.contains("serviceidentifier") {
                    if let value = rawValue as? String { out["serviceIdentifier"] = value }
                } else if key.contains("externalcontentidentifier") || key.contains("asseturl") {
                    if let value = rawValue as? String { out["contentIdentifier"] = value }
                }
            }
